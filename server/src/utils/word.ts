...
const isoToLanguage = Object.fromEntries(
  Object.entries(Languages).map(([key, val]) => [val, key])
);

function resolveLanguageCode(language: Languages): string {
  return isoToLanguage[language] ?? language;
}
...
function loadWords(language: Languages): Promise<string[]> {
  ...
  return new Promise((resolve, reject) => {
    if (wordsCache[langCode]) {
      return resolve(wordsCache[langCode]);
    }

    const filePath = path.join(WORDS_DIR, `${langCode}.txt`);
